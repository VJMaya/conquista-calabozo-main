...
function serializeTeamMember(member) {
  return {
    ...
    totalTimeSeconds: member.totalTimeSeconds || 0,
...
function serializeTeam(team) {
  return {
    ...
    name: team.name,
    teamName: team.name,
    members: getTeamMembers(team).map(serializeTeamMember),
...
function emitTeamMemberAnswered(team, player, isCorrect, pointsAwarded) {
  emitToTeam(team, 'team:member_answered', {
    teamId: team.id,
    userId: player.id,
...
function advanceTeamIfReady(team, timedOut = false) {
  ...
  (result.timeoutMisses || []).forEach((player) => {
    emitTeamMemberAnswered(team, player, false, 0);
  });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('player:submit_answer', (data = {}) => {
      ...
      socket.emit('answer:result', {
        ...
        answeredCount: user.answeredCount || 0,
        correctCount: user.correctCount || 0,
      });

      emitTeamMemberAnswered(team, user, result.isCorrect, result.pointsAwarded);
