...
function recordPlayerAnswer(team, player, submittedAnswer, now) {
  ...
  const wasActive = isActivePlayer(player);
...
function participatingMemberCount(team) {
  return Array.isArray(team.members) ? team.members.length : 0;
}

function activePlayerCountForTeam(team) {
  return Math.max(1, team.activePlayerCount || 0);
}

function normalizedScoreForTeam(team) {
  return (team.totalCorrect || 0) / (activePlayerCountForTeam(team) * TOTAL_QUESTIONS);
}
